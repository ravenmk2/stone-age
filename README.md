...
| --- | --- | --- |
| `ww-brainstorming` | Divergent research — surface alternatives, converge on a decision. | conclusion (no file) |
| `ww-exploring` | General research — refine a rough idea into a scoped conclusion. | conclusion (no file) |
| `ww-analyzing` | Diagnostic research — reproduce, locate root cause, define fix area. | conclusion (no file) |
| `ww-planning` | Write an executable Plan: scope + tasks + doc-change targets (development). | `docs/plans/active/YYYY-MM-DD-<slug>/` |
| `ww-executing` | Execute a Plan's tasks — verify and commit each, manage in-flight changes. | code + `plan.md` amendments |
...
| `ww-archiving` | Code-bearing terminal — merge spec/design into canonical docs (diff-reviewed), archive the Plan, final-commit. | merged docs + `docs/plans/completed/` |
